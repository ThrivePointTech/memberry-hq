...
export default async function MerchantDetailPage({
  ...
  const { merchant, plans, balance, active_payout_account, subscription_counts } = detail
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 gap-4 md:grid-cols-4">
        ...
        <Card>
          ...
          <CardContent>
            <p className="text-2xl font-semibold">{formatNumber(subscription_counts.active ?? 0)}</p>
          </CardContent>
        </Card>

        <Card>
          <CardHeader className="flex flex-row items-center justify-between pb-1">
            <CardTitle className="text-sm text-zinc-500">Payout Method</CardTitle>
            <PayoutAccountEdit merchantId={merchant.id} currentAccount={active_payout_account} />
          </CardHeader>
          <CardContent>
            {active_payout_account ? (
              <div>
                <p className="font-medium capitalize">{active_payout_account.method_type}</p>
                <p className="text-sm text-zinc-500">{active_payout_account.account_name}</p>
                <p className="text-xs text-zinc-400">{active_payout_account.account_number}</p>
              </div>
            ) : (
      ...
      <div>
        ...
        <QrTile
          title={merchant.name}
          subtitle="Shop QR"
          url={`https://getmemberry.com/shop/${merchant.id}`}
